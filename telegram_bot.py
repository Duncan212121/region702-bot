...
TYPES = {
    ...
    "inspected":   ("🔍", "Осмотрен, ждём оплату",  "Авто найден по запросу клиента, осмотрен специалистом в Китае, клиент СОГЛАСОВАЛ заказ. Промежуточный этап ДО выкупа. Структура: 🔍 «Найден и осмотрен по запросу клиента!» → состояние авто по осмотру (честно) → «Клиент согласовал ✅» → «Готовим инвойс, ждём оплату через ВТБ» → CTA «Ищете похожий? Пишите — подберём и проверим»"),
...
HINTS = {
    ...
    "inspected":  "Напиши данные осмотра:\n\nПример: Honda CR-V 2022 для клиента Динара, осмотр пройден, состояние подтверждено, клиент согласовал — готовим инвойс",
